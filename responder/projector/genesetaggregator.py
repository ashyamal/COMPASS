...
class GeneSetPoolingAggregator(nn.Module):
    ...
    def forward(self, gene_output):
        ...
        aggregated_features = torch.zeros(batch_size, num_genesets, feature_dim, device=gene_output.device)
...
class GeneSetAttentionAggregator(nn.Module):
    def __init__(self, genesets_indices):
        """
    ...
    def forward(self, gene_features):
        ...
        for i, gene_set in enumerate(self.genesets_indices):
            set_features = gene_features[:, gene_set, :]  # Extract features for genes in the set
