...
def plot_embed_with_label(dfp, label_col = ['cancer_type'],  label_type = ['c'], 
    ...
    if len(glist) == 2:
        ...
        mp = UMAP(spread=spread, min_dist=min_dist, n_neighbors=n_neighbors, metric=metric, random_state=random_state,  verbose=1, **kwargs) #, metric='correlation'
