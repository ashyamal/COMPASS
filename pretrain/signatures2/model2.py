...
class GeneEmbedder(nn.Module):
    def __init__(self, num_genes, emb_dim, with_bias = True):
        '''
        embedding genes
        '''
        super().__init__()
...
class TransformerEncoder(nn.Module):
    def __init__(self, input_dim, d_model = 32, num_layers = 2):
        super().__init__()

        self.embedder = GeneEmbedder(num_genes = input_dim, emb_dim = d_model, with_bias = False)
        self.encoder = Transformer(dim = d_model, depth = num_layers, 
                                   heads = 4, dim_head = 16, 
