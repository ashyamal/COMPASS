...
CELLTYPE_palette = pd.DataFrame([CONCEPT_palette]).T.reset_index().sort_index(ascending=False).set_index('index')[0].to_dict()
...
def plot_sankey_diagram(model, concept2plot = ['NKcell'],  reverse = False, scale_imp = False,  title_text = '',
                concept_imp_dict = dict(zip(features, [1 for i in features])), topK_vis = 10,
                font_size=15, width=1200, height=600, margin=dict(l=10, r=1, t=50, b=10), **layout_args):
    ...
    for i, (concept, idx) in enumerate(zip(concept_name, concept_idx)):
        ...
        for j, fi in zip(idx, geneset_w):
            ...
            gene_w = gene_w.numpy().reshape(-1,)*fi
    ...
    labels = df.groupby('source').weights.sum().sort_values(ascending=False).index.to_list()
    labels.extend(list(set(all_labels) - set(labels)))


    tail_nodes = df.groupby(['group', 
                             'source']).weights.sum().sort_values(ascending=False).loc['gene->geneset'].iloc[topK_vis:].index
