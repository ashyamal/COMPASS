...
class TripletCosineLoss(nn.Module):
    ...
    def forward(self, anchor, positive, negative):
        ...
        loss = torch.relu(self.margin - cos_sim_pos + cos_sim_neg).mean()
        return loss


class MSEWithNaNLabelsLoss(nn.Module):
    def __init__(self):
        super(MSEWithNaNLabelsLoss, self).__init__()

    def forward(self, predictions, labels):
        mask = ~torch.isnan(labels)[:, 0]
        mse_loss = torch.mean((predictions[mask] - labels[mask]) ** 2)
        return mse_loss
